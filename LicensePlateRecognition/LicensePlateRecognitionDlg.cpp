...
std::map< yolo_list, CString> CLicensePlateRecognitionDlg::m_MapForChooseDetectorType = { {YOLOV2,"Yolo V2"},{YOLOV3,"Yolo V3"},{YOLOV3_MOD,"YoloV3 Mod"},{YOLOV4,"Yolo v4"} };
...
BOOL CLicensePlateRecognitionDlg::OnInitDialog()
{
	...
	m_hRotateImageSlideBar.SetRange(-45, 45,TRUE);
	...
	m_hLightingImageSlideBar.SetRange(-45, 45,TRUE);
	m_hLightingImageSlideBar.SetPos(0);
	//m_hLightingImageNumShow.Format("%d", m_hRotateImageSlideBar.GetPos());
...
void CLicensePlateRecognitionDlg::OnDrawObject()
{
	RECT r;
	m_hImageControl.GetClientRect(&r);
	cv::Size winSize(640, 480);
	cv::Mat cvImgTmp(winSize, CV_8UC3);
	if (m_hImage.size() != winSize)
	{
		cv::resize(m_hImage, cvImgTmp, winSize);
	}
	else
	{
		cvImgTmp = m_hImage.clone();
	}
