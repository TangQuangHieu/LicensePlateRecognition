...
{
	m_pszPathName = NULL;
	CString pathName = MgGetModuleFileName();
	CString strExt = MgGetFileExt(pathName);
	pathName.Replace(strExt, ".ini");
...
BOOL CIni::WriteArray(LPCTSTR lpSection, LPCTSTR lpKey, const CStringArray *pArray, int nWriteCount, LPCTSTR lpDelimiter) const
{
	...
	if (nWriteCount < 0)
		...
		nWriteCount = __min(nWriteCount, (int)pArray->GetSize());
...
LPTSTR CIni::__StrDupEx(LPCTSTR lpStart, LPCTSTR lpEnd)
{
    ...
    const DWORD LEN = (lpEnd - lpStart) / sizeof(TCHAR);
    LPTSTR psz = new TCHAR[LEN + 1];
